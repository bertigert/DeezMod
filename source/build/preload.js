(() => {
    ...
    function getRealPath(app, pathname) {
        return isWindowsStore_isWindowsStore() ? (appPath && realAppPath || (appPath = app.getAppPath(), realAppPath = external_fs_default().realpathSync(appPath)), appPath !== realAppPath ? pathname.replace(realAppPath, appPath) : pathname) : pathname
    }
    require("i18next-sprintf-postprocessor");
    require("i18next-node-fs-backend");

    function isDev_isDev(app) {
    ...
    external_i18next_default().init({
        lng: getLanguage(remote_namespaceObject.app)
        ...
        constructor() {
            this.ipc = renderer_di.get(SERVICE_IPC), this.autoStart = renderer_di.get(SERVICE_AUTOSTART), this.flags = {
                ...
                this.reloadApp()
            }), remote_namespaceObject.powerMonitor.on("lock-screen", () => this.flags.disableNotification = !0), remote_namespaceObject.powerMonitor.on("unlock-screen", () => this.flags.disableNotification = !1)
        }
        ...
        getLang() {
            return getLanguage(remote_namespaceObject.app)
        }
        getAppVersion() {
            return remote_namespaceObject.app.getVersion()
        }
        getElectronVersion() {
        ...
        setAutoStart(enabled) {
            this.autoStart.setAutoStart(enabled, remote_namespaceObject.app)
        }
        isOpenAtLogin() {
            return this.autoStart.isOpenAtLogin(remote_namespaceObject.app)
        }
        getCookie(cookieName) {
            return new Promise((resolve, reject) => {
                remote_namespaceObject.session.defaultSession.cookies.get({}).then(cookies => {
                    let found = !1;
        ...
        setCookie(name, value) {
            ...
            return remote_namespaceObject.session.defaultSession.cookies.set(cookie)
        }
        getOriginalUserAgent() {
            return remote_namespaceObject.session.defaultSession.getUserAgent()
        }
        getSentryTags() {
            return {
                environment: isDev_isDev ? "development" : "production",
        ...
        getAppPath() {
            return getRealPath(remote_namespaceObject.app, external_path_default().join(remote_namespaceObject.app.getAppPath(), "build"))
        }
        getAssetsPath() {
            return getRealPath(remote_namespaceObject.app, external_path_default().join(remote_namespaceObject.app.getAppPath(), "build/assets"))
        }
    ...
    let plugins_loaded = async function load_plugins() {
        ...
        const {
            get_settings
        } = require("./DeezMod/settings");
        const prefix = location.pathname.endsWith("titlebar.html") ? "titlebar" : "renderer";
        const context = prefix + "Preload";
        get_settings(context);
        const plugins_promise = load_plugins(context, false);
        try {
            Object.defineProperty(window, "DeezMod", {
                value: {
                    plugins: plugins_promise
                },
                writable: false,
                configurable: false,
                enumerable: true
        ...
        const plugins = await plugins_promise;
        if (typeof plugins === "object" && plugins !== null) Object.freeze(plugins);
        const functions_in_scope = await execute_plugins(plugins[context], context);
        if (functions_in_scope !== "") {
            try {
                eval(functions_in_scope)
            } catch (e) {
                console.error("Error executing plugins in preload context with loader scope", e)
            }
        }
    }();
    module.exports = {}
