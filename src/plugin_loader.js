...
function log(...args) {
  console.log("[Plugin Loader]", ...args);
}
function error(...args) {
  console.error("[Plugin Loader]", ...args);
}
...
async function load_plugins(context, delete_cache = false) {
  ...
  if (!fs.existsSync(directory_path)) {
    ...
    log(`Created directory for plugins: ${directory_path}`);
    return plugin_cache;
  }
  is_loading = new Promise((resolve, reject) => {
    fs.readdir(directory_path, (err, files) => {
      ...
      js_files.forEach((file) => {
        ...
        log(`Loading file from context ${context}: ${file_path}`);
        if (delete_cache) {
          log("Invalidating cache for file:", file_path);
        ...
        try {
          ...
          error("Error loading file:", file_path, e);
        }
        if (plugin) {
...
function execute_plugins(plugins, context) {
  ...
  plugins?.forEach((plugin) => {
    log(`Evaluating plugin ${plugin.name} in the ${context} context`);
    if (plugin.scope[plugin.context.indexOf(context)] === "loader") {
      log("Using loader scope for plugin " + plugin.name);
      functions_in_scope += `(${plugin.func.toString()})("${context}");`;
    } else {
      log("Using plugin scope for plugin " + plugin.name);
      try {
        plugin.func(context);
      } catch (e) {
        error("Error executing plugin " + plugin.name, e);
      }
    }
  });
